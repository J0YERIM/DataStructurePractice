...
void StackChar::printStack() {
	if (!isEmpty()) {
		for (int i = 0; i <= getTop(); i++)
			if (getArr()[i] == '\0') printf("e ");
			else printf("%c ", getArr()[i]);
	}
}

typedef enum { lparen, rparen, plus, minus, times, divide, mod, power, nnot, eos, operand } precedence;
StackInt stkInt;
StackChar stkChar;
/* isp와 icp 배열 -- 인덱스는 연산자
lparen, rparen, plus, minus, times, divide, mod, eos의 우선순위 값*/
int isp[] = { 0,19,12,12,13,13,13, 15, 17, 0};
int icp[] = { 20,19,12,12,13,13,13, 15, 17, 0};
...
void postfix(char* expr, char* new_expr) {
	...
	for (token = getToken(expr, &symbol, &n); token != eos; token = getToken(expr, &symbol, &n)) {
		// eos일 때까지 입력 받기
		if (token == operand) {
			new_expr[i++] = symbol; // 피연산자일 경우 new_expr에 저장
		}
